...
/* Not using SPI for this example.
 * Using serial communication only.
 *
 * DIN: PA8  (output pin)
 * CS_PIN: PA9 (output pin)
 * CLK_PIN: PA4 (output pin)
 */

#define BUFF_SIZE 100

uint8_t buffer[BUFF_SIZE];
uint8_t rowBuffer[BUFF_SIZE];

/* the lookup table for many symbols */
uint8_t SYMBOLS[] = {
...
void DotDisplayInit(uint8_t brightness)
{
	...
	// 1. Exit display test mode FIRST
	DotDisplayWriteCommand(0x0F, 0x00);
	// 2. Exit shutdown mode
	DotDisplayWriteCommand(0x0C, 0x01);
	// 3. Disable BCD decode
	DotDisplayWriteCommand(0x09, 0x00);
	// 4. Set scan limit to all 8 rows
	DotDisplayWriteCommand(0x0B, 0x07);
	// 5. Set brightness
	DotDisplayWriteCommand(0x0A, brightness);
	// 6. Clear display
	DotDisplayClear();
}
...
void DotMatrixGpioInit(void)
{
    ...
    /* set PA8 (DIN) as output */
    GPIOA->MODER |= GPIO_MODER_MODER8_0;
    GPIOA->MODER &= ~(GPIO_MODER_MODER8_1);
    /* set PA9 (CS_PIN) as output */
    GPIOA->MODER |= GPIO_MODER_MODER9_0;
    GPIOA->MODER &= ~(GPIO_MODER_MODER9_1);
    /* set PA4 (CLK_PIN) as output */
...
void DotMatrixWriteByte(uint8_t byte)
{
	for (int i = 0; i < 8; ++i)
	{
		/* set DIN first while CLK is still low */
		if (byte & 0x80)
			GPIOA->BSRR = GPIO_BSRR_BS8;   // DIN high
		else
			GPIOA->BSRR = GPIO_BSRR_BR8;   // DIN low

		/* CLK high — latch the bit */
		GPIOA->BSRR = GPIO_BSRR_BS4;

		/* CLK low — ready for next bit */
...
void DotDisplayWriteCommand(uint8_t address, uint8_t command)
{
    ...
    GPIOA->BSRR = GPIO_BSRR_BR9;  // CS low

    /* to write at all displays */
    for(int i = 0; i < NUM_OF_DISPLAYS; ++i)
    {
        DotMatrixWriteByte(address);
        DotMatrixWriteByte(command);
    }
...
{
    WRITE_BIT(buffer[column], row, value);

    /* get the device number and column number */
    uint8_t displayNumber = column / 8;
    uint8_t columnNumber = column % 8;
    ...
    for(int i = 0; i < NUM_OF_DISPLAYS; ++i)
    {
        if(i == (NUM_OF_DISPLAYS - (displayNumber + 1)))
        {
            DotMatrixWriteByte((columnNumber + 1));
            DotMatrixWriteByte(buffer[column]);
        }
        else
        {
            DotMatrixWriteByte(0);
            DotMatrixWriteByte(0);
        }
    }
	/* pull the CS line low */
...
{
    /* get the device number and row number */
    uint8_t displayNumber = row / 8;
    uint8_t rowNumber = row % 8;

	/* update the row buffer */
	rowBuffer[row] = value;

	/* single CS-framed transaction for all 4 displays */
	GPIOA->ODR &= ~(GPIO_ODR_OD9);   // CS low
    ...
    for(int i = 0; i < NUM_OF_DISPLAYS; ++i)
    {
        if(i == displayNumber)
        {
        	DotMatrixWriteByte(rowNumber + 1);  // register address (1-8)
        	DotMatrixWriteByte(value);
        }
        else
        {
            DotMatrixWriteByte(0x00);
            DotMatrixWriteByte(0x00);
        }
    }
	GPIOA->ODR |= (GPIO_ODR_OD9);    // CS high — latch
}
...
void DotDisplayWriteCharacter(char c, uint8_t displayPosition)
{
	...
	{
		DotDisplaySetRow((row + (8*(displayPosition - 1))), SYMBOLS[i]);
		row--;
	}
}
...
void DotDisplayScrollCharacter(char c, uint8_t speed, char direction)
{
	...
	switch(direction)
	{
		case ('L'):
			...
			{
				/* set row */
				DotDisplaySetRow(row, SYMBOLS[i]);
				--row;
			}
			/* shift left */
		...
		case ('R'):
			for (int i = start; i < (start + width + 1); ++i)
			{
				DotDisplaySetRow(row + ((NUM_OF_DISPLAYS - 1)*8), SYMBOLS[i]);
				--row;
			}
			/* shift right */
			for (int i = 0; i < (NUM_OF_DISPLAYS * 8); ++i)
			{
				DotDisplayShiftRight();
				delay(speed);
			}
		break;
...
void DotMatrixShiftCharater(char c, uint32_t speed, char direction)
{
	...
	switch (direction)
	{
		...
		{
			DotDisplaySetRow((NUM_OF_DISPLAYS * 8) - 1, SYMBOLS[i]);
			DotDisplayShiftRight();
			delay(speed);
		}
		break;
...
void DotDisplayScrollString(uint8_t *str, uint32_t speed, char direction)
{
	while (*str != 0)
	{
		DotMatrixShiftCharater(*str, speed, direction);
		++str;
	}
}
...
{
	for (int i = 0; i < (NUM_OF_DISPLAYS * 8) - 1; ++i)
	{
		rowBuffer[i] = rowBuffer[i - 1];
	}
	rowBuffer[(NUM_OF_DISPLAYS * 8) - 1] = 0;

	// Redraw all rows from the updated buffer
	for (int i = 0; i < NUM_OF_DISPLAYS * 8; ++i)
	{
		DotDisplaySetRow(i, rowBuffer[i]);
	}
...
{
	int last = (NUM_OF_DISPLAYS * 8) - 1;  //we have 8 columns in each display
	uint8_t old = rowBuffer[0];
	...
	rowBuffer[(NUM_OF_DISPLAYS * 8) - 1] = old;
}
